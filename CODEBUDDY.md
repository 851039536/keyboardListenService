...
这是整个项目中最重要的类，位于 `SN_DesktopService` 命名空间（使用文件级命名空间声明），修饰符为 `public static`，完全静态。
...
**停止流程（`Stop`）**：
- 设置 `_isRunning = false` 停止回调事件触发
- 调用 `UnhookWindowsHookEx` 卸载钩子（不立即置零 `_hookID`，等消息线程退出后再清除）
- 如果后台消息线程存活，调用 `Application.Exit()` 退出消息循环
- 等待线程最多 500ms，然后置零 `_hookID`

### HotkeyService —— 业务逻辑层

命名空间 `SN_DesktopService`，`public static` 类，持有静态字段 `_form1` 作为 Form1 的单例引用，`_started` 标志位防重入。

**事件处理器逻辑**：
- `OnKeyDown`：用 `Control.ModifierKeys == Keys.Control` 判断 Ctrl 是否按下（区别于检查 `Keys.ControlKey`，`ModifierKeys` 工作在全局钩子线程中同样有效）
- `OnKeyUp`：输出释放日志

**Ctrl+K 单例逻辑**：如果 `_form1` 不为 null 且未释放（`IsDisposed`），直接返回；否则创建新实例，订阅 `FormClosed` 事件以在关闭时置 null，然后 `Show()` 非模态显示。

### 潜在问题与注意事项

1. **静态事件生命周期**：`HotkeyService` 为静态类，订阅 `M_GlobalKeyListener` 事件后进程运行期间始终存活，无事件泄漏风险。
2. **权限要求**：`WH_KEYBOARD_LL` 在某些 Windows 安全策略下可能需要管理员权限才能正常工作。钩子安装失败时会输出 Win32 错误码。
3. **优雅退出**：`Program.cs` 注册了 `Console.CancelKeyPress`，按 `Ctrl+C` 时自动调用 `M_GlobalKeyListener.Stop()` 卸载钩子后退出。
4. **`.csproj` 项目名 vs 文件夹名**：项目文件名为 `keyboardListenService.csproj`，但文件夹名为 `SN_DesktopService`。解决方案中的项目显示名为 `keyboardListenService`。
